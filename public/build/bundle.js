...
var app = (function () {
    ...
    function create_fragment$9(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(svg, file$9, 65, 2, 1859);
    			attr_dev(div, "id", "chart");
    			attr_dev(div, "class", "svelte-t7rmx2");
    			add_location(div, file$9, 64, 0, 1840);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    // (78:27)      <span style="display:none;"       >{setContext("plansdata", {         allYears: plansContext(normalizeArray(data)),         fullData: normalizeArray(data),       }
    function create_then_block(ctx) {
    	let span;

    	let t0_value = setContext("plansdata", {
    		allYears: plansContext(normalizeArray(/*data*/ ctx[2])),
    		fullData: normalizeArray(/*data*/ ctx[2])
    	}) + "";
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "then",
    		source: "(78:27)      <span style=\\\"display:none;\\\"       >{setContext(\\\"plansdata\\\", {         allYears: plansContext(normalizeArray(data)),         fullData: normalizeArray(data),       }",
    		ctx
    	});
    ...
    function create_fragment$b(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(span, file$b, 71, 4, 2143);
    			add_location(div0, file$b, 70, 2, 2133);
    			attr_dev(div1, "class", "app-header r-mono flex-c center-first svelte-112fmhv");
    			add_location(div1, file$b, 69, 0, 2079);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function instance$b($$self, $$props, $$invalidate) {
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ("sortedData" in $$props) $$invalidate(10, sortedData = $$props.sortedData);
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*$plans, data, sortedData, dataParam*/ 1031) {
    			onMount(async () => {
    				setTimeout(() => $$invalidate(3, loading = false), 3000);
    				$$invalidate(2, data = await $plans);
    				$$invalidate(10, sortedData = normalizeArray(data));
    				console.log("sorted", data);
    				$$invalidate(0, dataParam = plansContext(sortedData)[0]);
    				yearParam.set(dataParam);
    				psVolume.set(getProductionSalesVolumePerYear(dataParam, sortedData));
    				revenue.set(getRevenuePerYear(dataParam, sortedData));
    				expense.set(getExpensePerYear(dataParam, sortedData));
